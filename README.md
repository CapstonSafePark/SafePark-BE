...
- **운영 (AWS EC2):** `http://52.79.227.161:8080`
...
`POST /api/analysis/upload-image` 호출 시 DS 서버 차선 감지 + 위치 정보를 조합해서 위험도 계산

| 차선 | 위치 | 위험도 | 확률 |
|------|------|--------|------|
| 황색 이중선 | 무관 | HIGH | 95% |
| 황색 단선 | 스쿨존/버스정류장 30m 내 | HIGH | 92% |
| 황색 단선 | 단속카메라 100m 내 | HIGH | 85% |
| 황색 단선 | 없음 | HIGH | 80% |
| 황색 점선 | 스쿨존/버스정류장 30m 내 | HIGH | 88% |
| 황색 점선 | 단속카메라 100m 내 | MEDIUM | 70% |
| 황색 점선 | 없음 | MEDIUM | 55% |
| 미감지/흰선 | 스쿨존/버스정류장 30m 내 | HIGH | 90% |
| 미감지/흰선 | 단속카메라 100m 내 | MEDIUM | 60% |
| 미감지/흰선 | 없음 | LOW | 20% |

DS 서버 응답 실패 시 위치 기반으로만 계산 (정상 응답 보장)
...
- 2026.05.08 - 의왕시 DB 데이터 적재 완료
- 2026.05.08 - 이미지 분석 로직 개선 (DS 차선 감지 + 위치 조합)
- 2026.05.08 - EC2 서버 IP 변경 (13.124.185.9 → 52.79.227.161)
